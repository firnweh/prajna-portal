    ...
    <div className="bg-prajna-card border border-prajna-border rounded-xl p-4">
      ...
      <ResponsiveContainer width="100%" height={Math.max(200, sorted.length * 32)}>
        <BarChart data={sorted} layout="vertical" margin={{ left: 10, right: 20 }}>
          ...
          <XAxis type="number" domain={[0, 100]} tick={{ fill: '#64748b', fontSize: 10 }} axisLine={{ stroke: '#1e1e3a' }} />
          <YAxis type="category" dataKey="name" width={100} tick={{ fill: '#e2e8f0', fontSize: 11, fontWeight: 600 }} axisLine={false} />
          <Tooltip contentStyle={{ background: '#1a1d2e', border: '1px solid #1e1e3a', borderRadius: 8, color: '#e2e8f0' }} formatter={(val) => [`${Number(val).toFixed(1)}%`, 'Avg Score']} />
